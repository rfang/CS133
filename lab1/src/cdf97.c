...
void fwt_test( float*x, float*tempbank, int n, int m, int N, int M) { 
	int i, j;
		...
		int N, int M)
{
	...
	for (i=0; i<n; i++) {
		...
		if( i%2 == 0) {
			for(j=0; j<m; j++) {
			tempbank[(i/2)*M+j] = k1*x[j*n+i];	
			}
		}
		else 
			for(j=0; j<m; j++) {
			tempbank[(i/2+n/2)*M+j] = k2*x[j*n+i];
			}
		...
		) 
{
	...
	if (level > 0) {	
		...
		for (i=0; i<nlevel; i++) {
			...
			fwt_test(x, tempbank, m, n, M, N);
			// Stage 3
			fwt97_pd(tempbank, n, m, n, m);
			//fwt_two(tempbank, x, n, m, N, M);
			// Stage 4
